...
def load_data():
    ...
    try:
        ...
        # Éviter division par zéro si un seul joueur joue
...
def compute_stats(df, bp_map, daily_max_map):
    ...
    # --- NOUVEAU : Calcul pour Sparklines (liste des scores) ---
    # On prend les 20 derniers picks pour la visual
    spark_map = df.sort_values('Pick').groupby('Player')['Score'].apply(lambda x: x.tail(20).tolist()).to_dict()
    ...
    for p in df['Player'].unique():
        ...
        # Moyenne Z-Score (Impact relatif)
        ...
        stats.append({
            ...
            'AvgZ': avg_z, # Nouvel indicateur
            'TrendData': spark_map.get(p, []) # Pour Sparkline
        })
...
try:
    ...
    if df is not None and not df.empty:
        ...
        with st.sidebar:
            ...
            st.markdown(f"""<div style='position: fixed; bottom: 30px; width: 100%; padding-left: 20px;'><div style='color:#444; font-size:10px; font-family:Rajdhani; letter-spacing:2px; text-transform:uppercase'>Data Pick #{int(latest_pick)}<br>War Room v2.0</div></div>""", unsafe_allow_html=True)
        ...
        if menu == "Dashboard":
            ...
            # ... (KPIs Cards identiques au V1 mais on garde le layout) ...
            with c_stats:
                ...
                with r1c3: st.markdown(f"<div class='stat-box-mini'><div class='stat-mini-val'>{total_nukes_team}</div><div class='stat-mini-lbl'>NUKES</div><div class='stat-mini-sub'>Scores > 50</div></div>", unsafe_allow_html=True)
            ...
            with c_info:
                ...
                <div class="glass-card" style="height:100%; display:flex; flex-direction:column; justify-content:space-around; padding:20px;">
                    ...
                    <div class="hq-card-row">
                        <div class="hq-lbl">🚀 PLAFOND</div>
                        <div class="hq-val" style="color:{C_GREEN}">{int(best_night)}</div>
                    </div>
                    <div class="hq-card-row">
                        <div class="hq-lbl">🧱 PLANCHER</div>
            ...
            # --- 🔥 NOUVEAUTÉ : HEATMAP ---
            st.markdown("### 🔥 VUE THERMIQUE (HEATMAP)")
            st.markdown("<div class='chart-desc'>L'historique complet en un coup d'oeil. Les zones rouges indiquent les carottes, les vertes les top scores.</div>", unsafe_allow_html=True)
            ...
            # --- 🔥 NOUVEAUTÉ : CONTRIBUTION ---
            st.markdown("### 🍰 PART DE CONTRIBUTION")
            st.markdown("<div class='chart-desc'>Qui porte l'équipe ? Vue normalisée à 100% de la contribution de chaque joueur par soir.</div>", unsafe_allow_html=True)
            # On filtre pour ne pas surcharger le graph (ex: les 20 derniers picks)
            contrib_df = df[df['Pick'] > (latest_pick - 20)]
            ...
            st.markdown("### 📊 DATA ROOM (SPARKLINES EDITION)")
            st.markdown("<div class='chart-desc'>Tableau avancé avec courbes de tendance intégrées (20 derniers matchs).</div>", unsafe_allow_html=True)
            
            # --- 🔥 NOUVEAUTÉ : SPARKLINES DATAFRAME ---
            st.dataframe(full_stats[['Player', 'TrendData', 'Total', 'Moyenne', 'BP_Count', 'Nukes', 'Carottes']].sort_values('Total', ascending=False), 
                         hide_index=True, 
                         use_container_width=True, 
                         column_config={
                            "Player": st.column_config.TextColumn("Joueur", width="medium"),
                            "TrendData": st.column_config.LineChartColumn("Tendance (20j)", width="medium", y_min=0, y_max=80),
                            "Total": st.column_config.ProgressColumn("Total", format="%d", min_value=0, max_value=full_stats['Total'].max()), 
                            "Moyenne": st.column_config.NumberColumn("Moy.", format="%.1f"),
                            "BP_Count": st.column_config.NumberColumn("🎯 BP"),
                            "Nukes": st.column_config.NumberColumn("☢️"),
                            "Carottes": st.column_config.NumberColumn("🥕")
                        })
            ...
            with c3: kpi_card("IMPACT RELATIF (Z)", f"{z_val:+.2f}", "VS MOYENNE TEAM", z_col)
            
            with c4: kpi_card("CLASSEMENT", f"#{internal_rank}", f"SUR {nb_players}", C_GOLD if internal_rank==1 else "#FFF")
            ...
            with c_trend:
                st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
                st.markdown("#### 📈 TENDANCE (15J)", unsafe_allow_html=True)
                ...
                if not last_15_data.empty:
                    ...
                    fig_trend.add_hline(y=p_data['Moyenne'], line_dash="dot", line_color=C_TEXT, annotation_text="Moy. Saison")
            ...
            if df_bonus.empty: st.info("Aucun bonus utilisé pour le moment.")
                ...
                st.markdown("### 💰 RENTABILITÉ DES BONUS")
                ...
                with c1:
                    ...
                    if hot.empty: h += "<div style='color:#666'>Aucune donnée</div>"
                        ...
                        for p, v in hot.head(3).items():
                            val = float(v)
                            vf = f"+{val:.1f}" if (metric in ["diff", "pct"] and val > 0) else f"{val:.1f}"
                            if metric == "pct": vf += "%"
            ...
            render_row("MOYENNE GÉNÉRALE (15 JOURS)", "Les joueurs les plus réguliers vs ceux en difficulté.", avg_15, avg_15.sort_values(), "raw", "pts")
            ...
            # Leader Z-Score
            mvp_z = full_stats.sort_values('AvgZ', ascending=False).iloc[0]
            ...
            with c1:
                ...
                st.markdown(hof_card("IMPACT PLAYER", "⚡", C_BLUE, mvp_z['Player'], f"{mvp_z['AvgZ']:.2f}", "Z-SCORE", "Meilleure performance relative à l'équipe"), unsafe_allow_html=True)
                st.markdown(hof_card("REAL MVP", "💎", C_PURE, pure_avg['Player'], f"{pure_avg['Moyenne_Raw']:.1f}", "PTS MOY (BRUT)", "Meilleure moyenne sans bonus"), unsafe_allow_html=True)
                st.markdown(hof_card("THE SNIPER", "🎯", C_PURPLE, sniper_bp['Player'], int(sniper_bp['BP_Count']), "BEST PICKS", "Plus grand nombre de Best Picks"), unsafe_allow_html=True)
                st.markdown(hof_card("ALPHA DOG", "🐺", C_ALPHA, alpha_dog['Player'], int(alpha_dog['Alpha_Count']), "TOPS TEAM", "Le plus souvent meilleur scoreur du jour"), unsafe_allow_html=True)
                st.markdown(hof_card("HUMAN TORCH", "🔥", "#FF5252", torche['Player'], f"{torche['Last15']:.1f}", "PTS / 15J", "Meilleure moyenne récente"), unsafe_allow_html=True)
                
            with c2:
                st.markdown(hof_card("THE CEILING", "🏔️", "#A78BFA", peak['Player'], int(peak['Best']), "PTS MAX", "Record absolu de points"), unsafe_allow_html=True)
                st.markdown(hof_card("THE ALCHEMIST", "⚗️", C_BONUS, alchemist['Player'], int(alchemist['Bonus_Gained']), "PTS BONUS", "Plus grand gain sur bonus"), unsafe_allow_html=True)
                st.markdown(hof_card("ZEN MASTER", "🧘", "#EAB308", zen_master['Player'], f"{int(zen_master['ReliabilityPct'])}%", "FIABILITÉ", "% de matchs sans carotte"), unsafe_allow_html=True)
                st.markdown(hof_card("NUCLEAR", "☢️", "#EF4444", nuke['Player'], int(nuke['Nukes']), "BOMBS", "Nombre de scores > 50 points"), unsafe_allow_html=True)
                st.markdown(hof_card("THE FARMER", "🥕", "#F97316", lapin['Player'], int(lapin['Carottes']), "CAROTTES", "Le roi du potager"), unsafe_allow_html=True)
            ...
            if not st.session_state["admin_access"]:
                ...
                with c2:
                    ...
                    # Aperçu du message
                    st.info("Aperçu : Un clic enverra le récapitulatif du dernier pick sur le channel.")
