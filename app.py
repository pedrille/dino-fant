...
# --- CONFIG COULEURS SCORES (NOUVEAU STANDARD v23) ---
C_CARROT = "#EF4444"  # 0-19 (Rouge Vif)
C_AVG    = "#374151"  # 20-39 (Gris Foncé - Neutre)
C_GOOD   = "#10B981"  # 40+ (Vert Émeraude)
C_GOLD   = "#FFD700"  # Bonus / Elite
...
# --- 2. CSS PREMIUM (V23 : AJOUT TICKER) ---
st.markdown(f"""
<style>
        ...
        if start_row_rank != -1:
            for i in range(start_row_rank+1, start_row_rank+30):
                ...
                if "Team Raptors" in p_name:
                    # CORRECTION v22: On scanne toute la ligne pour éviter le bug des 2 points
...
def compute_stats(df, bp_map, daily_max_map):
    ...
    for p in df['Player'].unique():
        ...
        # --- V23: THE PHOENIX (Best score after carrot) ---
        max_phoenix = 0
        for i in range(len(scores) - 1):
            if scores[i] < 20: # Carrot logic
                rebound = scores[i+1]
                if rebound > max_phoenix: max_phoenix = rebound

        # --- V23: ROLLING 7 BEST WEEK ---
        max_rolling_7 = 0
        if len(scores) >= 7:
...
try:
    ...
    if df is not None and not df.empty:
        ...
        <div class="ticker-wrap">
            <div class="ticker">
                <div class="ticker__item">{ticker_text}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if menu == "Dashboard":
            ...
            with c_gen:
                ...
                for i, r in top_5_season.iterrows():
                    ...
                    evo = f"<span style='color:{C_GOOD}; font-size:0.8rem'>▲{int(diff)}</span>" if diff > 0 else (f"<span style='color:{C_CARROT}; font-size:0.8rem'>▼{int(abs(diff))}</span>" if diff < 0 else "<span style='color:#444; font-size:0.8rem'>=</span>")
            ...
            with c_form:
                st.markdown(f"<div class='glass-card' style='height:100%'><div style='color:{C_GOOD}; font-family:Rajdhani; font-weight:700; margin-bottom:5px'>🔥 TOP 5 FORME (15J)</div><div class='chart-desc'>Meilleures moyennes sur les 15 derniers picks.</div>", unsafe_allow_html=True)
                top_5_form = full_stats.sort_values('Last15', ascending=False).head(5).reset_index()
                for i, r in top_5_form.iterrows():
                    medal = medals.get(i, f"{i+1}")
                    st.markdown(f"<div style='display:flex; align-items:center; justify-content:space-between; margin-bottom:8px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:6px'><div style='display:flex; align-items:center; gap:10px'><div style='font-size:1.2rem; width:20px'>{medal}</div><div style='font-family:Rajdhani; font-weight:600; font-size:1rem; color:#FFF'>{r['Player']}</div></div><div style='font-family:Rajdhani; font-weight:700; color:{C_GOOD}'>{r['Last15']:.1f}</div></div>", unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)
            ...
            col_dyn = C_GOOD if (avg_team_15 - daily_totals.mean()) > 0 else C_ORANGE
            ...
            col_dyn_team = C_GOOD if diff_dyn_team > 0 else C_CARROT
            ...
            with c_grid:
                ...
                with g5: st.markdown(f"<div class='stat-box-mini'><div class='stat-mini-val' style='color:{C_GOOD}'>{total_nukes_team}</div><div class='stat-mini-lbl'>NUKES (> 50 PTS)</div></div>", unsafe_allow_html=True)
                with g6: st.markdown(f"<div class='stat-box-mini'><div class='stat-mini-val' style='color:{C_CARROT}'>{total_carrots_team}</div><div class='stat-mini-lbl'>CAROTTES (< 20 PTS)</div></div>", unsafe_allow_html=True)
            ...
            with c_rec:
                st.markdown(f"""<div class="glass-card" style="height:100%; display:flex; flex-direction:column; justify-content:center; padding:25px;"><div style="text-align:center; margin-bottom:20px; font-family:Rajdhani; font-weight:700; font-size:1.1rem; color:#AAA; letter-spacing:1px; border-bottom:1px solid #333; padding-bottom:10px;">RECORDS & MOYENNE</div><div class="hq-card-row"><div class="hq-lbl">🚀 RECORD</div><div class="hq-val" style="color:{C_GOOD}">{int(best_night)}</div></div><div class="hq-card-row"><div class="hq-lbl">⚖️ MOYENNE</div><div class="hq-val">{int(avg_night)}</div></div><div class="hq-card-row"><div class="hq-lbl">🧱 PLANCHER</div><div class="hq-val" style="color:{C_ACCENT}">{int(worst_night)}</div></div></div>""", unsafe_allow_html=True)

            st.markdown("<div style='margin-bottom:30px'></div>", unsafe_allow_html=True)
            
            # --- V23: NEW CHARTS (Bell Curve & Bar Race) ---
            # Suppression du graph historique classement buggé
            
            c_bc1, c_bc2 = st.columns(2, gap="medium")
            ...
            with c_bc1:
                ...
                # Utilisation de Plotly histogramme simple pour la courbe
                fig_bell = px.histogram(df, x="Score", nbins=20, color_discrete_sequence=[C_BLUE], opacity=0.7, marginal="box")
                fig_bell.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font={'color': '#AAA'}, height=400, xaxis_title="Score", yaxis_title="Volume")
                st.plotly_chart(fig_bell, use_container_width=True)
                
            with c_bc2:
                st.markdown("### 🏁 BAR CHART RACE (ANIMÉE)")
                st.markdown("<div class='chart-desc'>Utilisez le slider en bas pour revoir la course au classement jour après jour.</div>", unsafe_allow_html=True)
                
                # Préparation Data pour Bar Race (Cumulatif)
                ...
                # Pour éviter de surcharger si trop de picks, on prend un échantillon tous les 2 picks si > 50
                if df_cumul['Pick'].nunique() > 50:
                    df_race = df_cumul[df_cumul['Pick'] % 2 == 0].copy()
                else:
                    df_race = df_cumul.copy()
                
                fig_race = px.bar(df_race, x="Total", y="Player", orientation='h', text="Total", 
                                  animation_frame="Pick", range_x=[0, df_race['Total'].max()*1.1])
                ...
                fig_race.update_layout(
                    ...
                    xaxis=dict(showgrid=True, gridcolor='#333'), margin=dict(l=0, r=0, t=0, b=0)
                )
                st.plotly_chart(fig_race, use_container_width=True)

            st.markdown("### 🔥 HEATMAP UNIFIÉE")
            st.markdown(f"<div class='chart-desc'>Rouge vif = Carotte. Vert vif = Carton.</div>", unsafe_allow_html=True)
            ...
            # V23: CUSTOM COLOR SCALE (Red -> Grey -> Green)
            custom_colors = [
                [0.0, C_CARROT], # 0
                [0.25, C_CARROT], # ~20
                [0.25, C_AVG], # 20
            ...
            st.dataframe(full_stats[['Player', 'TrendArrow', 'Total', 'Moyenne', 'BP_Count', 'Nukes', 'Carottes', 'Bonus_Gained']].sort_values('Total', ascending=False), hide_index=True, use_container_width=True, column_config={
                ...
                "TrendArrow": st.column_config.TextColumn("Tendance", width="small", help="Basé sur les 5 derniers matchs"),
            ...
            if comp_player != "Aucun":
                ...
                with c1:
                    ...
                    # COMPARATOR TABLE
                    metrics = [
                        ("TOTAL POINTS", 'Total', 'int'),
                        ("MOYENNE", 'Moyenne', 'float'),
                        ("RECORD", 'Best', 'int'),
                        ("NUKES (>50)", 'Nukes', 'int'),
                        ("CAROTTES (<20)", 'Carottes', 'int'),
                        ("FORME (15J)", 'Last15', 'float')
                    ]
                    ...
                    for label, key, fmt in metrics:
                        ...
                        # Win Logic
                        win1 = v1 > v2
                        if key == 'Carottes': win1 = v1 < v2 # Less carrots is better
                        ...
                        st.markdown(f"""
                        <div class="h2h-row">
                            <div class="h2h-val" style="color:{col1}">{val1}</div>
                            <div class="h2h-label">{label}</div>
                            <div class="h2h-val" style="color:{col2}">{val2}</div>
                        </div>
                        """, unsafe_allow_html=True)
                ...
                with c2:
                    ...
                    def get_radar_vals(d):
                        reg_score = 100 - ((d['StdDev'] / full_stats['StdDev'].max()) * 100)
                        return [
                            (d['Moyenne'] / max_avg) * 100, 
                            (d['Best'] / max_best) * 100, 
                            (d['Last10'] / full_stats['Last10'].max()) * 100, 
                            reg_score, 
                            (d['Nukes'] / (full_stats['Nukes'].max() if full_stats['Nukes'].max()>0 else 1)) * 100
                        ]
                    
                    r_cats = ['SCORING', 'CEILING', 'FORME', 'RÉGULARITÉ', 'CLUTCH']
                    
                    fig_radar = go.Figure()
                    fig_radar.add_trace(go.Scatterpolar(r=get_radar_vals(p_data)+[get_radar_vals(p_data)[0]], theta=r_cats+[r_cats[0]], fill='toself', name=sel_player, line_color=PLAYER_COLORS.get(sel_player, C_ACCENT)))
                    fig_radar.add_trace(go.Scatterpolar(r=get_radar_vals(comp_data)+[get_radar_vals(comp_data)[0]], theta=r_cats+[r_cats[0]], fill='toself', name=comp_player, line_color=PLAYER_COLORS.get(comp_player, C_BLUE), opacity=0.5))
                    
                    fig_radar.update_layout(
                        polar=dict(radialaxis=dict(visible=True, range=[0, 100], showticklabels=False), bgcolor='rgba(0,0,0,0)'),
                        paper_bgcolor='rgba(0,0,0,0)', font=dict(color='white'), margin=dict(t=20, b=20, l=40, r=40), height=400,
                ...
                color_diff = C_GOOD if diff_form > 0 else C_CARROT
                z_val = p_data['CurrentNoCarrot']
                z_col = C_GOOD if z_val > 3 else (C_CARROT if z_val == 0 else "#FFF")
                ...
                # --- NEW ROW: SEASON HISTORY VISUALIZER (WITH NEW COLORS) ---
                ...
                with col_grid:
                    ...
                    with r3c2: st.markdown(f"<div class='stat-box-mini'><div class='stat-mini-val' style='color:{C_GOOD}'>{int(p_data['Nukes'])}</div><div class='stat-mini-lbl'>NUKES (> 50 PTS)</div></div>", unsafe_allow_html=True)
                    with r3c3: st.markdown(f"<div class='stat-box-mini'><div class='stat-mini-val' style='color:{C_CARROT}'>{int(p_data['Carottes'])}</div><div class='stat-mini-lbl'>CAROTTES (< 20 PTS)</div></div>", unsafe_allow_html=True)
                ...
                with c_radar_legend:
                    st.markdown("""<div class='legend-box'><div class='legend-item'><div class='legend-title'>SCORING</div><div class='legend-desc'>Volume de points moyen.</div></div><div class='legend-item'><div class='legend-title'>CEILING</div><div class='legend-desc'>Potentiel max sur un match.</div></div><div class='legend-item'><div class='legend-title'>FORME</div><div class='legend-desc'>Dynamique actuelle (10j).</div></div><div class='legend-item'><div class='legend-title'>RÉGULARITÉ</div><div class='legend-desc'>Capacité à éviter les écarts.</div></div><div class='legend-item'><div class='legend-title'>CLUTCH</div><div class='legend-desc'>Fréquence des scores > 50 pts.</div></div></div>""", unsafe_allow_html=True)

                # --- V23: SCATTER PLOT RISK/REWARD ---
                st.markdown("<br>", unsafe_allow_html=True)
                c_scat, c_dist = st.columns(2)
                
                with c_scat:
                    st.markdown("#### 🎲 RISQUE vs RÉCOMPENSE")
                    st.markdown("<div class='chart-desc'>Positionnement du joueur par rapport à l'équipe.</div>", unsafe_allow_html=True)
                    # Create scatter plot with all players, highlighting selected
                    full_stats['Color'] = np.where(full_stats['Player'] == sel_player, C_GOLD, '#555')
                    full_stats['Size'] = np.where(full_stats['Player'] == sel_player, 15, 8)
                    
                    fig_scat = px.scatter(full_stats, x="Moyenne", y="StdDev", text="Player", hover_name="Player")
                    fig_scat.update_traces(marker=dict(color=full_stats['Color'], size=full_stats['Size'], line=dict(width=1, color='White')), textposition='top center')
                    fig_scat.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font={'color': '#AAA'}, height=350, xaxis_title="Performance (Moyenne)", yaxis_title="Instabilité (Écart-Type)")
                    st.plotly_chart(fig_scat, use_container_width=True)

                with c_dist:
                    st.markdown("#### 📊 DISTRIBUTION DES SCORES")
                    st.markdown("<div class='chart-desc'>Fréquence des scores sur la période.</div>", unsafe_allow_html=True)
                    fig_hist = px.histogram(p_hist_all, x="Score", nbins=15, color_discrete_sequence=[C_ACCENT], text_auto=True)
                    fig_hist.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font={'color': '#AAA'}, margin=dict(l=0, r=0, t=10, b=0), height=350, xaxis_title=None, yaxis_title=None, bargap=0.1)
            ...
            if df_bonus_disp.empty: st.info("Aucun bonus trouvé.")
                ...
                with c_ch2:
                    ...
                    fig_strip.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font={'color': '#AAA'}, height=300, xaxis=dict(title=None), showlegend=False)
            ...
            fig_car.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font={'color': '#AAA'}, xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor='#222'), coloraxis_showscale=False, height=350)
            ...
            fig_team_15.update_traces(line_color=C_ACCENT, line_width=3)
            fig_team_15.add_hline(y=season_avg_team, line_dash="dot", line_color=C_TEXT)
            fig_team_15.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font={'color': '#AAA'}, xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor='#222'), height=350)
            ...
            # V23: AJOUT DES NOUVEAUX BADGES ET EMOJIS UNIQUES
            hof_list = [
                ...
                {"title": "THE PHOENIX", "icon": "🐣", "color": "#F43F5E", "p": get_hof('MaxPhoenix'), "v": int(get_hof('MaxPhoenix')['MaxPhoenix']), "u": "REBOND", "d": "Meilleur score lendemain de carotte"},
                ...
                {"title": "PURE SCORER", "icon": "🏀", "color": "#7C3AED", "p": get_hof('Best_Raw'), "v": int(get_hof('Best_Raw')['Best_Raw']), "u": "MAX BRUT", "d": "Record sans bonus"},
                {"title": "THE ALCHEMIST", "icon": "⚗️", "color": C_BONUS, "p": get_hof('Bonus_Gained'), "v": int(get_hof('Bonus_Gained')['Bonus_Gained']), "u": "PTS BONUS", "d": "Gains via bonus"},
                ...
                {"title": "RISING STAR", "icon": "🚀", "color": "#34D399", "p": get_hof('ProgressionPct'), "v": f"+{get_hof('ProgressionPct')['ProgressionPct']:.1f}%", "u": "PROGRESSION", "d": "Hausse forme vs saison"},
                ...
                {"title": "THE MANIAC", "icon": "🤪", "color": "#D946EF", "p": get_hof('ModeCount'), "v": f"{get_hof('ModeCount')['ModeScore']}", "u": "REPETITION", "d": "Score le plus répété"},
                {"title": "IRON WALL", "icon": "🧱", "color": "#78350F", "p": get_hof('Worst'), "v": int(get_hof('Worst')['Worst']), "u": "PIRE SCORE", "d": "Plancher le plus haut"},
                {"title": "ALBATROSS", "icon": "🦅", "color": "#2DD4BF", "p": get_hof('Spread'), "v": int(get_hof('Spread')['Spread']), "u": "AMPLITUDE", "d": "Ecart Max vs Min"},
                ...
                {"title": "BAD BUSINESS", "icon": "💸", "color": "#9CA3AF", "p": get_hof('Bonus_Gained', True), "v": int(get_hof('Bonus_Gained', True)['Bonus_Gained']), "u": "GAINS", "d": "Le moins de gain bonus"},
                {"title": "THE BRICK", "icon": "🏗️", "color": "#6B7280", "p": get_hof('Worst_Raw', True), "v": int(get_hof('Worst_Raw', True)['Worst_Raw']), "u": "MIN BRUT", "d": "Le pire score absolu"},
