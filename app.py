...
try:
    ...
    if df is not None and not df.empty:
        ...
        with st.sidebar:
            ...
            st.markdown(f"""<div style='position: fixed; bottom: 30px; width: 100%; padding-left: 20px;'><div style='color:#444; font-size:10px; font-family:Rajdhani; letter-spacing:2px; text-transform:uppercase'>Data Pick #{int(latest_pick)}<br>War Room v12.2 (ProGradient)</div></div>""", unsafe_allow_html=True)
        ...
        if menu == "Dashboard":
            ...
            # --- AJOUT: HEATMAP (Couleurs Corrigées Asymétriques) ---
            st.markdown("### 🔥 HEATMAP DE LA SAISON (Asymétrique)")
            st.markdown("<div class='chart-desc'>Rouge < 30 pts | Gris ~ 42 pts | Vert > 45 pts</div>", unsafe_allow_html=True)
            ...
            fig_heat = px.imshow(
                ...
                color_continuous_scale=[
                    ...
                    [0.4, '#1F2937'],   # ~25 pts -> Sombre/Rouge
                    [0.55, '#374151'],  # ~41 pts -> Gris Neutre (Zone Tampon)
                    [0.75, '#10B981'],  # ~55 pts -> Vert
                    [1.0, '#10B981']    # Max -> Vert Flash
                ], 
                aspect="auto"
            )
