...
# --- PALETTE COULEURS JOUEURS (12 Couleurs distinctes pour mode sombre) ---
PLAYER_PALETTE = [
    "#CE1141", # Raptors Red
    "#FFD700", # Gold
    "#3B82F6", # Blue
...
<style>
    ...
    .pb-name {{ font-family: 'Rajdhani'; font-size: 2rem; font-weight: 800; color: #FFF; line-height: 1; text-shadow: 0 2px 10px rgba(0,0,0,0.5); }}
    .pb-sub {{ font-family: 'Rajdhani'; font-size: 1rem; font-weight: 600; color: rgba(255,255,255,0.8); text-transform: uppercase; letter-spacing: 2px; }}
    ...
    /* Match Pills */
    .match-pill {{
        flex: 1 0 auto; min-width: 25px; height: 35px; border-radius: 4px; 
        display: flex; align-items: center; justify-content: center; 
        font-family: 'Rajdhani'; font-weight: 700; font-size: 0.8rem; color: #FFF; margin: 0 1px;
    }}
    .match-row {{ display: flex; justify-content: space-between; width: 100%; gap: 2px; margin: 15px 0; overflow-x: auto; }}
    ...
    /* TOP 5 PICKS STYLING */
    .top-pick-row {{ display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.03); border-left: 4px solid {C_ACCENT}; padding: 12px 15px; margin-bottom: 8px; border-radius: 4px; transition: background 0.2s; }}
...
def load_data():
    ...
    try:
        ...
        unique_players = sorted(final_df['Player'].unique().tolist())
        player_colors = {}
        for i, player in enumerate(unique_players):
            # Cycle through palette if more players than colors (modulo)
            player_colors[player] = PLAYER_PALETTE[i % len(PLAYER_PALETTE)]
        ...
        if start_row_rank != -1:
            for i in range(start_row_rank+1, start_row_rank+30):
                ...
                if "Team Raptors" in p_name:
                    ...
                    break
        return final_df, team_current_rank, bp_map, team_rank_history, daily_max_map, player_colors, team_bp_real
...
try:
    ...
    with st.spinner('🦖 Analyse des données en cours...'):
        df, team_rank, bp_map, team_history, daily_max_map, player_colors, team_bp_real_load = load_data()
    
    if df is not None and not df.empty:
        team_avg_per_pick = df['Score'].mean()
    else: team_avg_per_pick = 0
    ...
    if df is not None and not df.empty:
        ...
        if menu == "Dashboard":
            ...
            total_nukes_team = len(df[df['Score'] >= 50]); total_carrots_team = len(df[df['Score'] < 20]); safe_zone_team = len(df[df['Score'] > 35])
            ...
            # --- PLAYER COLORED BANNER ---
            p_color = player_colors.get(sel_player, C_ACCENT)
            st.markdown(f"""
            <div class="player-banner" style="background: linear-gradient(90deg, {p_color}CC 0%, {p_color}33 100%); border-left: 6px solid {p_color};">
                <div>
                    <div class="pb-sub">RAPTORS ROSTER</div>
                    <div class="pb-name">{sel_player.upper()}</div>
                </div>
                <div style="font-family:Rajdhani; font-size:2.5rem; opacity:0.2; font-weight:800">#{int(p_data['Total'])}</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            form_10 = p_data['Last10']; form_15 = p_data['Last15']
            diff_form = ((form_15 - p_data['Moyenne']) / p_data['Moyenne']) * 100
            sign = "+" if diff_form > 0 else ""; color_diff = C_GREEN if diff_form > 0 else "#F87171"
            z_val = p_data['CurrentNoCarrot']; z_col = C_GREEN if z_val > 3 else (C_RED if z_val == 0 else "#FFF")
            ...
            last_30_picks = last_30_picks.sort_values('Pick', ascending=True)
            
            if not last_30_picks.empty:
                html_picks = "<div class='match-row' style='width:100%'>"
                for _, r in last_30_picks.iterrows():
                    sc = r['Score']; bg = C_GOLD if r['IsBonus'] else (C_RED if sc < 20 else (C_GREEN if sc > 40 else "#333"))
                    txt_col = "#000000" if r['IsBonus'] else "#FFF"; border = f"2px solid {C_GOLD}" if r['IsBonus'] else "1px solid rgba(255,255,255,0.1)"
            ...
            else: st.info("Pas encore assez de matchs joués.")
            ...
            with col_top5:
                ...
                for i, r in top_5.reset_index().iterrows():
                    ...
                    st.markdown(f"""<div class="top-pick-row"><div class="tp-rank">#{i+1}</div><div class="tp-info"><div class="tp-pick">Pick #{r['Pick']}</div><span class="tp-bonus">{b_icon}</span></div><div class="tp-score">{int(r['Score'])}</div></div>""", unsafe_allow_html=True)
            ...
            with c_trend:
                if not p_hist_all.empty:
                    ...
                    if not last_15_data.empty:
                        ...
                        fig_trend.update_traces(line_color=C_GOLD, marker_color=p_color, marker_size=8)
                    ...
                    else: st.info("Pas assez de données récentes.")
            ...
            if not p_hist_all.empty:
                ...
                fig_evol.update_traces(line_color=p_color, line_width=2, marker_size=4)
            ...
            if df_bonus_disp.empty: st.info("Aucun bonus trouvé pour cette sélection.")
                ...
                nb_bonus = len(df_bonus_disp); avg_bonus = df_bonus_disp['Score'].mean()
                total_gain = df_bonus_disp['RealGain'].sum(); success_rate = (len(df_bonus_disp[df_bonus_disp['Score'] >= 40]) / nb_bonus * 100) if nb_bonus > 0 else 0
                ...
                with c_chart1:
                    ...
                    fig_m.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font={'color': '#AAA'}, xaxis=dict(title=None), yaxis=dict(showgrid=False, visible=False), height=350, showlegend=False, coloraxis_showscale=False)
                    st.plotly_chart(fig_m, use_container_width=True)
                
                with c_chart2:
                    st.markdown("#### 🎯 IMPACT PAR JOUEUR (STRIP PLOT)")
                    st.markdown("<div class='chart-desc'>Chaque point représente un bonus joué. Superpositions = Mêmes scores.</div>", unsafe_allow_html=True)
                    # NEW STRIP PLOT WITH ASSIGNED COLORS
                    fig_strip = px.strip(df_bonus_disp, x='Player', y='Score', color='Player', stripmode='overlay', color_discrete_map=player_colors)
                    fig_strip.add_hline(y=40, line_dash="dash", line_color="#666", annotation_text="Seuil Rentabilité", annotation_position="bottom right")
                    fig_strip.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font={'color': '#AAA'}, height=350, xaxis_title=None, yaxis_title="Score Total", showlegend=False)
                    # Update marker size
                    fig_strip.update_traces(marker=dict(size=12, line=dict(width=1, color='white')))
                    st.plotly_chart(fig_strip, use_container_width=True)

                st.markdown("### 📜 HISTORIQUE DÉTAILLÉ")
                st.dataframe(df_bonus_disp[['Pick', 'Player', 'Month', 'ScoreVal', 'Score', 'RealGain']].sort_values('Pick', ascending=False), hide_index=True, use_container_width=True, column_config={"Pick": st.column_config.NumberColumn("Pick #", format="%d"), "ScoreVal": st.column_config.NumberColumn("Score Brut", format="%d pts"), "Score": st.column_config.NumberColumn("Score Final (x2)", format="%d pts"), "RealGain": st.column_config.NumberColumn("Gain Net", format="+%d pts")})

        elif menu == "Trends":
            section_title("TENDANCES", "Analyse de la forme récente (15 derniers jours)")
            df_15 = df[df['Pick'] > (latest_pick - 15)]
            team_daily_15 = df_15.groupby('Pick')['Score'].sum(); avg_15_team = team_daily_15.mean()
            team_daily_season = df.groupby('Pick')['Score'].sum(); season_avg_team = team_daily_season.mean()
            team_trend_diff = ((avg_15_team - season_avg_team) / season_avg_team) * 100
            best_form_player = df_15.groupby('Player')['Score'].mean().idxmax(); best_form_val = df_15.groupby('Player')['Score'].mean().max()
            avg_15_indiv = df_15['Score'].mean(); max_team_15 = team_daily_15.max()
            ...
            col_trend = C_GREEN if team_trend_diff > 0 else C_RED; sign_trend = "+" if team_trend_diff > 0 else ""
            ...
            delta_df['Delta'] = delta_df['Last15'] - delta_df['Season']; delta_df = delta_df.dropna()
            ...
            with c_hot:
                ...
                if hot_players.empty: st.info("Personne n'est en sur-régime actuellement.")
                else:
                    for p, row in hot_players.iterrows(): st.markdown(f"<div class='trend-card-row'><div class='trend-name'>{p}</div><div style='text-align:right'><div class='trend-val' style='color:{C_GREEN}'>{row['Last15']:.1f}</div><div class='trend-delta' style='color:{C_GREEN}'>+{row['Delta']:.1f}</div></div></div>", unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)
            with c_cold:
                ...
                if cold_players.empty: st.info("Tout le monde tient son rang !")
                else:
                    for p, row in cold_players.iterrows(): st.markdown(f"<div class='trend-card-row'><div class='trend-name'>{p}</div><div style='text-align:right'><div class='trend-val' style='color:{C_RED}'>{row['Last15']:.1f}</div><div class='trend-delta' style='color:{C_RED}'>{row['Delta']:.1f}</div></div></div>", unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)
            ...
            # UPDATE : USE PLAYER COLORS HERE
            fig_mom = px.line(momentum_data, x='Pick', y='Score', color='Player', markers=True, color_discrete_map=player_colors)
            fig_mom.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font={'color': '#AAA'}, xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor='#222'), height=500, legend=dict(orientation="h", y=-0.2))
            ...
            if not st.session_state["admin_access"]:
                ...
                with c2:
                    ...
                    if st.button("DÉVERROUILLER"):
                        if "ADMIN_PASSWORD" in st.secrets and pwd == st.secrets["ADMIN_PASSWORD"]: st.session_state["admin_access"] = True; st.rerun()
