import pandas as pd
import numpy as np
import datetime # IMPORTANT : Il faut cet import pour le timedelta
...
def generate_weekly_report_data(df_full):
    ...
    # --- CORRECTION DATE ---
    # On prend la date MIN du deck (le Lundi)
    start_date_obj = week_df['Date'].min()
    # On force la date de fin à J+6 (le Dimanche), peu importe les matchs joués
    ...
    for i, (player, score) in enumerate(weekly_scores.head(3).items()):
        ...
        rank_disp = i + 1
        if i == 0 and score == weekly_scores.iloc[0]: nb_titles += 1
        elif i > 0 and score == weekly_scores.iloc[i-1]: rank_disp = weekly_podium[-1]['rank']
    ...
    # Sunday Clutch (Dernier Pick du Deck)
