...
# Date du Pick #1 = Mardi 21 Octobre 2025 (Votre date officielle)
...
def generate_weekly_report_data(df_full, target_deck_num=None):
    ...
    # --- 1. DATES RÉELLES 2025 ---
    first_pick = week_df['Pick'].min()
    last_pick = week_df['Pick'].max()
    
    # On calcule les dates exactes basées sur le calendrier 2025
    ...
    stats_week = stats_week.sort_values('mean', ascending=False) # Classement Moyenne
    ...
    for i, (player, row) in enumerate(stats_week.head(3).iterrows()):
        ...
        # Le titre ROTW reste au plus gros TOTAL (Règle officielle)
