...
<style>
    ...
    .kpi-card-premium:hover {{ transform: translateY(-2px); border-color: rgba(255,255,255,0.15); }}
    .kpi-icon-bg {{ position: absolute; right: -10px; top: -10px; font-size: 4rem; opacity: 0.05; pointer-events: none; }}
    .kpi-title {{ font-size: 0.75rem; color: #888; text-transform: uppercase; letter-spacing: 1px; font-weight: 600; }}
    .kpi-value {{ font-family: 'Rajdhani'; font-size: 2.2rem; font-weight: 800; color: #FFF; line-height: 1.2; margin-top: 5px; }}
    .kpi-unit {{ font-size: 0.7rem; font-weight: 700; text-transform: uppercase; margin-top: 2px; }}
...
# --- 3. DATA ENGINE (V9 - OPTIMIZED & ROBUST) ---
...
def load_data():
    ...
    try:
        ...
        # CALCUL DU SCORE : On applique x2 si bonus
...
# --- 5. UI COMPONENTS ---
# NOUVELLE FONCTION CARD PREMIUM UNIFIÉE
def kpi_card_premium(title, value, unit, color, icon=""):
    st.markdown(f"""
    <div class="kpi-card-premium" style="border-bottom: 2px solid {color};">
...
try:
    ...
    if df is not None and not df.empty:
        ...
        with st.sidebar:
            ...
            st.markdown(f"""<div style='position: fixed; bottom: 30px; width: 100%; padding-left: 20px;'><div style='color:#444; font-size:10px; font-family:Rajdhani; letter-spacing:2px; text-transform:uppercase'>Data Pick #{int(latest_pick)}<br>War Room v9.0</div></div>""", unsafe_allow_html=True)

        if menu == "Dashboard":
            section_title("RAPTORS <span class='highlight'>DASHBOARD</span>", f"Daily Briefing • Pick #{int(latest_pick)}")
            top = day_df.iloc[0]
            c1, c2, c3, c4 = st.columns(4)
            with c1: kpi_card_premium("MVP DU JOUR", top['Player'], f"{int(top['Score'])} PTS", C_GOLD, "👑")
            with c2: kpi_card_premium("MOYENNE TEAM", int(day_df['Score'].mean()), "POINTS", "#FFF", "📊")
            ...
            with c3: kpi_card_premium("CLASSEMENT TEAM", r_disp, "GENERAL", c_rank, "🌍")
            with c4: kpi_card_premium("LEADER SAISON", leader['Player'], f"TOTAL: {int(leader['Total'])}", C_ACCENT, "🦁")
            ...
            with col_stats:
                # NEW PREMIUM CARDS FOR PLAYER LAB
                c1, c2 = st.columns(2)
                with c1: kpi_card_premium("MOYENNE SAISON", f"{p_data['Moyenne']:.1f}", "PTS / MATCH", "#FFF", "📈")
                rank_col = C_GOLD if internal_rank == 1 else (C_SILVER if internal_rank == 2 else (C_BRONZE if internal_rank == 3 else "#FFF"))
                with c2: kpi_card_premium("CLASSEMENT TEAM", f"#{internal_rank}", f"SUR {nb_players}", rank_col, "🏅")
            ...
            if df_bonus.empty: st.info("Aucun bonus utilisé pour le moment.")
                ...
                k1, k2, k3 = st.columns(3)
                with k1: kpi_card_premium("BONUS JOUÉS", nb_bonus, "TOTAL SAISON", C_BONUS, "🃏")
                with k2: kpi_card_premium("MOYENNE BONUS", f"{avg_bonus:.1f}", "POINTS DOUBLÉS", "#FFF", "📈")
                with k3: kpi_card_premium("MEILLEUR BONUS", int(best_bonus), "RECORD", C_GOLD, "🚀")
