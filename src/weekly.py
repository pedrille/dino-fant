...
def generate_weekly_report_data(df_full, target_deck_num=None):
    ...
    # Check Completeness (avant tout le reste)
    # On récupère le plan théorique du Deck depuis le loader
    expected_picks = df_full.attrs.get('deck_tracks', {}).get(target_deck, [])
    is_complete = True
    
    period_str = f"Picks #{first_pick} à #{last_pick}"
    
    if expected_picks:
        last_expected = max(expected_picks)
        if last_expected not in week_df['Pick'].unique():
            is_complete = False
            period_str += f" (Attente Pick #{last_expected})"
    ...
    # --- 1. HISTORIQUE & VAINQUEUR ACTUEL (CORRECTION DU BUG) ---
    rotw_history = {}
    ...
    # IDENTIFICATION DU VAINQUEUR DECK ACTUEL
    current_total_scores = week_df.groupby('Player')['Score'].sum()
    if not current_total_scores.empty:
        current_winner = current_total_scores.idxmax()
        # ON AJOUTE LE TITRE MAINTENANT (Avant de faire les listes)
        rotw_history[current_winner] = rotw_history.get(current_winner, 0) + 1
    else:
        current_winner = None
    ...
    for i, (player, row) in enumerate(stats_week.head(3).iterrows()):
        # On récupère le total à jour (incluant le titre actuel)
        nb_rotw = rotw_history.get(player, 0)
        is_official_winner = (player == current_winner)
    ...
    sunday_winners = []
    # Sunday Clutch (seulement si le deck est fini ou qu'on a des données sur le dernier jour)
    last_pick_data = week_df[week_df['Pick'] == last_pick]
    if not last_pick_data.empty:
        sunday_winners = get_winners_list(last_pick_data.groupby('Player')['Score'].max())
