...
<style>
    ...
    .kpi-sub {{ font-size: 0.75rem; color: #888; text-transform: uppercase; letter-spacing: 1px; margin-top: 4px; }}
    ...
    /* V12.0: highlight utilisé dans les titres */
...
try:
    ...
    if df is not None and not df.empty:
        ...
        # --- V12.0: quelques stats globales utiles un peu partout ---
        total_days_played = int(df['Pick'].nunique())
        ...
        with st.sidebar:
            ...
            menu = option_menu(
                menu_title=None,
                options=["Dashboard", "Team HQ", "Player Lab", "Bonus x2", "Trends", "Hall of Fame", "Admin"],
                icons=["grid-fill", "people-fill", "person-bounding-box", "lightning-charge-fill", "fire", "trophy-fill", "shield-lock"],
                default_index=0,
                styles={
            ...
            st.markdown(
                f"""<div style='position: fixed; bottom: 30px; width: 100%; padding-left: 20px;'>
                        <div style='color:#444; font-size:10px; font-family:Rajdhani; letter-spacing:2px; text-transform:uppercase'>
                            Data Pick #{int(latest_pick)}<br>War Room v12.0
                        </div>
                    </div>""",
                unsafe_allow_html=True
            )
        ...
        if menu == "Dashboard":
            ...
            with c1:
                kpi_card("MVP DU JOUR", top['Player'], f"{int(top['Score'])} PTS", C_GOLD)
            
            total_day = day_df['Score'].sum()
            with c2:
                kpi_card("TOTAL TEAM JOUR", int(total_day), "POINTS")
            ...
            with c3:
                kpi_card("PERF. TEAM JOUR", f"{diff_perf:+.1f}%", "VS MOY. SAISON", perf_col)
            
            with c4:
                kpi_card("LEADER SAISON", leader['Player'], f"TOTAL: {int(leader['Total'])}", C_ACCENT)
            ...
            with col_chart:
                ...
                fig.update_layout(
                    ...
                    xaxis=dict(title=None, tickfont=dict(size=14, family='Rajdhani', weight=600)),
                    height=350,
                    showlegend=False,
            ...
            with col_rank:
                ...
                for i, r in day_df.reset_index().iterrows():
                    ...
                    style = (
                        f"border-left: 3px solid {C_ACCENT}; background: rgba(255,255,255,0.03);"
                        if i == 0 else
                        "border-left: 3px solid transparent;"
                    )
                    col = C_ACCENT if i == 0 else "#FFF"
                    bonus_icon = " 🔥" if r['IsBonus'] else ""
                    rows += f"""<div class='rank-row' style='{style}'>
                                    <div class='rank-pos'>{pos}</div>
                                    <div class='rank-name' style='color:{"#FFF" if i < 3 else "#AAA"}'>
                                        {r['Player']}{bonus_icon}
                                    </div>
                                    <div class='rank-score' style='color:{col}'>{int(r['Score'])}</div>
                                </div>"""
            ...
            with c_podium:
                st.markdown(
                    ...
                    f"<div style='color:{C_ACCENT}; font-family:Rajdhani; font-weight:700; margin-bottom:15px'>"
                    f"🏆 LEADERS SAISON (TOTAL PTS)</div>",
                    unsafe_allow_html=True
                )
                ...
                for i, r in top_3_season.iterrows():
                    ...
                    <div style='display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;
                        ...
                        <div style='display:flex; align-items:center; gap:15px'>
                            ...
                            <div style='font-family:Rajdhani; font-weight:700; font-size:1.2rem; color:#FFF'>
                                {r['Player']}
                            </div>
                        </div>
                        <div style='font-family:Rajdhani; font-weight:700; font-size:1.5rem;
                                    color:{C_ACCENT if i==0 else "#FFF"}'>
                            {int(r['Total'])}
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
            ...
            with c_forme:
                st.markdown(
                    f"<div class='glass-card' style='height:100%'>"
                    f"<div style='color:{C_GREEN}; font-family:Rajdhani; font-weight:700; margin-bottom:15px'>"
                    f"🔥 FORME DU MOMENT (MOY. 15J)</div>",
                    unsafe_allow_html=True
                )
            ...
            # --- V12.0: petit résumé "discipline" du pick du jour ---
            day_carrots = int((day_df['Score'] < 20).sum())
            day_nukes = int((day_df['Score'] >= 50).sum())
            day_nopicks = int((day_df['Score'] == 0).sum())
            st.markdown("<div style='margin-top:20px'></div>", unsafe_allow_html=True)
            st.markdown(
                f"**Résumé du pick #{int(latest_pick)} :** "
                f"{day_nukes} nukes • {day_carrots} carottes • {day_nopicks} NoPick",
            ...
            with c_stats:
                ...
                with r1c1:
                    st.markdown(
                        f"<div class='stat-box-mini'>"
                        f"<div class='stat-mini-val' style='color:{C_GOLD}'>{int(total_pts_season)}</div>"
                        f"<div class='stat-mini-lbl'>TOTAL SAISON</div>"
                        f"<div class='stat-mini-sub'>Points Cumulés</div></div>",
                        unsafe_allow_html=True
                    )
                with r1c2:
                    st.markdown(
                        f"<div class='stat-box-mini'>"
                        f"<div class='stat-mini-val' style='color:{C_BONUS}'>{total_bonus_played}</div>"
                        f"<div class='stat-mini-lbl'>BONUS JOUÉS</div>"
                        f"<div class='stat-mini-sub'>Total Équipe</div></div>",
                        unsafe_allow_html=True
                    )
                with r1c3:
                    st.markdown(
                        f"<div class='stat-box-mini'>"
                        f"<div class='stat-mini-val'>{avg_bonus_team:.1f}</div>"
                        f"<div class='stat-mini-lbl'>EFFICIENCE</div>"
                        f"<div class='stat-mini-sub'>Moyenne sous Bonus</div></div>",
                        unsafe_allow_html=True
                    )
                ...
                with r2c1:
                    st.markdown(
                        f"<div class='stat-box-mini'>"
                        f"<div class='stat-mini-val' style='color:{C_ACCENT}'>{total_nukes_team}</div>"
                        f"<div class='stat-mini-lbl'>TOTAL NUKES</div>"
                        f"<div class='stat-mini-sub'>Sur {total_picks_played} picks</div></div>",
                        unsafe_allow_html=True
                    )
                with r2c2:
                    st.markdown(
                        f"<div class='stat-box-mini'>"
                        f"<div class='stat-mini-val' style='color:{C_ORANGE}'>{total_carrots_team}</div>"
                        f"<div class='stat-mini-lbl'>TOTAL CAROTTES</div>"
                        f"<div class='stat-mini-sub'>Sur {total_picks_played} picks</div></div>",
                        unsafe_allow_html=True
                    )
                with r2c3:
                    st.markdown(
                        f"<div class='stat-box-mini'>"
                        f"<div class='stat-mini-val'>{int(nb_matchs_joues)}</div>"
                        f"<div class='stat-mini-lbl'>MATCHS JOUÉS</div>"
                        f"<div class='stat-mini-sub'>Jours de compétition</div></div>",
                        unsafe_allow_html=True
                    )
                st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
                ...
                with r3c1:
                    st.markdown(
                        f"<div class='stat-box-mini'>"
                        f"<div class='stat-mini-val'>{best_rank_ever}</div>"
                        f"<div class='stat-mini-lbl'>MEILLEUR RANG</div>"
                        f"<div class='stat-mini-sub'>Historique (Maj Hebdo)</div></div>",
                        unsafe_allow_html=True
                    )
                ...
                with r3c2:
                    st.markdown(
                        f"<div class='stat-box-mini'>"
                        f"<div class='stat-mini-val'>{team_avg_per_pick:.1f}</div>"
                        f"<div class='stat-mini-lbl'>MOY. / PICK</div>"
                        f"<div class='stat-mini-sub'>Global Saison</div></div>",
                        unsafe_allow_html=True
                    )
                with r3c3:
                    st.markdown(
                        f"<div class='stat-box-mini'>"
                        f"<div class='stat-mini-val' style='color:{col_dyn}'>{int(avg_team_15)}</div>"
                        f"<div class='stat-mini-lbl'>DYNAMIQUE (15J)</div>"
                        f"<div class='stat-mini-sub'>vs {int(avg_team_season)} (Saison)</div></div>",
                        unsafe_allow_html=True
                    )
            ...
            fig_team_trend.add_hline(
                y=team_season_avg_total,
                line_dash="dot",
                line_color=C_TEXT,
                annotation_text="Moy. Totaux Saison",
            ...
            st.dataframe(
                full_stats[['Player', 'Total', 'Moyenne', 'BP_Count', 'Nukes', 'Carottes', 'Bonus_Gained']]
                ...
                column_config={
                    "Total": st.column_config.ProgressColumn(
                        "Total Pts", format="%d",
                        min_value=0,
                        max_value=full_stats['Total'].max()
                    ), 
            ...
            # --- V12.0: HEATMAP des scores par joueur & pick ---
            st.markdown("### 🔥 HEATMAP DES SCORES PAR JOUEUR")
            st.markdown(
                "<div class='chart-desc'>Chaque case correspond au score d'un joueur pour un pick donné. "
                "Permet de repérer d'un coup d'œil les séries chaudes ou compliquées.</div>",
                unsafe_allow_html=True
            )
            try:
                heat_df = df.pivot_table(index='Player', columns='Pick', values='Score', aggfunc='mean')
                heat_df = heat_df.sort_index()
                fig_heat = px.imshow(
                    heat_df,
                    aspect="auto",
                    color_continuous_scale=[C_BG, C_ACCENT],
                    labels=dict(x="Pick #", y="Joueur", color="Score")
                )
                ...
                st.info(f"Heatmap indisponible ({e})")
            ...
            c1, c2, c3, c4 = st.columns(4)
            with c1:
                kpi_card("TOTAL POINTS", int(p_data['Total']), "SAISON")
            with c2:
                kpi_card("MOYENNE", f"{p_data['Moyenne']:.1f}", "PTS / PICK")
            rank_col = (
                C_GOLD if internal_rank == 1 else
                (C_SILVER if internal_rank == 2 else
                 (C_BRONZE if internal_rank == 3 else "#FFF"))
            )
            with c3:
                kpi_card("CLASSEMENT", f"#{internal_rank}", f"SUR {nb_players}", rank_col)
            with c4:
                kpi_card("BEST SCORE", int(p_data['Best']), "RECORD", C_GOLD)
            ...
            for i, (idx, r) in enumerate(p_hist.iterrows()):
                ...
                cols[i].markdown(
                    f"<div style='text-align:center; font-family:Rajdhani; font-size:0.9rem; font-weight:800; "
                    f"color:{c}; background:rgba(255,255,255,0.05); border:{border}; border-radius:6px; "
                    f"padding:6px 2px'>{int(s)}</div>",
                    unsafe_allow_html=True
                )
            st.markdown("<br>", unsafe_allow_html=True)
            ...
            with col_radar:
                ...
                max_avg = full_stats['Moyenne'].max()
                max_best = full_stats['Best'].max()
                max_last5 = full_stats['Last5'].max()
                max_nukes = full_stats['Nukes'].max()
                reg_score = 100 - ((p_data['StdDev'] / full_stats['StdDev'].max()) * 100)
                r_vals = [
                    (p_data['Moyenne'] / max_avg) * 100 if max_avg > 0 else 0,
                    (p_data['Best'] / max_best) * 100 if max_best > 0 else 0,
                    (p_data['Last5'] / max_last5) * 100 if max_last5 > 0 else 0,
                    reg_score,
            ...
            with col_stats:
                ...
                with r1c1:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val'>{int(p_data['ReliabilityPct'])}%</div>"
                        f"<div class='stat-mini-lbl'>FIABILITÉ</div>"
                        f"<div class='stat-mini-sub'>% Picks > 20pts</div></div>",
                        unsafe_allow_html=True
                    )
                with r1c2:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val'>{int(p_data['Nukes'])}</div>"
                        f"<div class='stat-mini-lbl'>NUKES</div>"
                        f"<div class='stat-mini-sub'>Scores > 50pts</div></div>",
                        unsafe_allow_html=True
                    )
                with r1c3:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val' style='color:{C_RED}'>{int(p_data['Carottes'])}</div>"
                        f"<div class='stat-mini-lbl'>CAROTTES</div>"
                        f"<div class='stat-mini-sub'>Scores < 20pts</div></div>",
                        unsafe_allow_html=True
                    )
                st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
                ...
                with r2c1:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val'>{int(sniper_pct)}%</div>"
                        f"<div class='stat-mini-lbl'>SNIPER RATE</div>"
                        f"<div class='stat-mini-sub'>% Best Pick</div></div>",
                        unsafe_allow_html=True
                    )
                with r2c2:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val'>{p_data['Avg_Bonus']:.1f}</div>"
                        f"<div class='stat-mini-lbl'>BONUS EFF.</div>"
                        f"<div class='stat-mini-sub'>Moyenne sous x2</div></div>",
                        unsafe_allow_html=True
                    )
                with r2c3:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val'>{int(p_data['Count40'])}</div>"
                        f"<div class='stat-mini-lbl'>HEAVY HITS</div>"
                        f"<div class='stat-mini-sub'>Scores > 40pts</div></div>",
                        unsafe_allow_html=True
                    )
                st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
                ...
                with r3c1:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val'>{p_data['Last10']:.1f}</div>"
                        f"<div class='stat-mini-lbl'>FORME (10j)</div>"
                        f"<div class='stat-mini-sub'>Moyenne récente</div></div>",
                        unsafe_allow_html=True
                    )
                with r3c2:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val' style='color:{color_diff}'>{sign}{diff_form:.1f}</div>"
                        f"<div class='stat-mini-lbl'>DYNAMIQUE</div>"
                        f"<div class='stat-mini-sub'>vs Moy. Saison</div></div>",
                        unsafe_allow_html=True
                    )
                with r3c3:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val'>{int(p_data['Worst'])}</div>"
                        f"<div class='stat-mini-lbl'>FLOOR</div>"
                        f"<div class='stat-mini-sub'>Pire Score</div></div>",
                        unsafe_allow_html=True
                    )
                st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
                ...
                with r4c1:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val' style='color:{C_GOLD}'>{int(alpha_rate)}%</div>"
                        f"<div class='stat-mini-lbl'>ALPHA %</div>"
                        f"<div class='stat-mini-sub'>Leader Team</div></div>",
                        unsafe_allow_html=True
                    )
                with r4c2:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val'>{p_data['Moyenne_Raw']:.1f}</div>"
                        f"<div class='stat-mini-lbl'>MOY. PURE</div>"
                        f"<div class='stat-mini-sub'>Sans Bonus</div></div>",
                        unsafe_allow_html=True
                    )
                with r4c3:
                    st.markdown(
                        f"<div class='stat-box-mini'><div class='stat-mini-val'>{int(p_data['Count30'])}</div>"
                        f"<div class='stat-mini-lbl'>SAFE ZONE</div>"
                        f"<div class='stat-mini-sub'>Scores > 30pts</div></div>",
                        unsafe_allow_html=True
                    )
            ...
            with c_trend:
                st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
                st.markmarkdown("#### 📈 TENDANCE (15 DERNIERS MATCHS)", unsafe_allow_html=True)
            ...
            # --- V12.0: PROFIL JOUEUR VS MOYENNE ÉQUIPE ---
            st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
            st.markdown("#### 🧬 PROFIL VS MOYENNE ÉQUIPE", unsafe_allow_html=True)
            st.markdown(
                "<div class='chart-desc'>Comparaison rapide du joueur avec la moyenne de l'équipe sur quelques indicateurs clés.</div>",
                unsafe_allow_html=True
            )
            team_ref = full_stats.mean(numeric_only=True)
            comp_cols = ["Moyenne", "Last15", "Nukes", "Carottes", "ReliabilityPct"]
            comp_labels = {
                "Moyenne": "Moy. Saison",
                "Last15": "Moy. 15 derniers picks",
                "Nukes": "Nukes (>=50)",
                "Carottes": "Carottes (<20)",
                "ReliabilityPct": "Fiabilité (%)"
            }
            rows_html = ""
            for col_key in comp_cols:
                if col_key not in p_data or col_key not in team_ref:
                    continue
                player_val = float(p_data[col_key])
                team_val = float(team_ref[col_key])
                diff_val = player_val - team_val
                diff_sign = "+" if diff_val >= 0 else ""
                diff_color = C_GREEN if diff_val >= 0 else C_RED

                # formatage simple
                if col_key in ["Nukes", "Carottes"]:
                    pv_str = f"{int(round(player_val))}"
                    tv_str = f"{team_val:.1f}"
                    dv_str = f"{diff_sign}{diff_val:.1f}"
                elif col_key == "ReliabilityPct":
                    pv_str = f"{player_val:.0f}%"
                    tv_str = f"{team_val:.0f}%"
                    dv_str = f"{diff_sign}{diff_val:.0f}%"
                else:
                    pv_str = f"{player_val:.1f}"
                    tv_str = f"{team_val:.1f}"
                    dv_str = f"{diff_sign}{diff_val:.1f}"

                rows_html += f"""
                <div class='hq-card-row'>
                    <div class='hq-lbl'>{comp_labels.get(col_key, col_key)}</div>
                    <div style='text-align:right'>
                        <div class='hq-val'>{pv_str}</div>
                        <div style='font-size:0.7rem; color:#888'>
                            Équipe: {tv_str} • <span style='color:{diff_color}'>{dv_str}</span>
                        </div>
                    </div>
                </div>
                """
            ...
            if df_bonus.empty:
                ...
                with k1:
                    kpi_card("BONUS JOUÉS", nb_bonus, "TOTAL SAISON", C_BONUS)
                with k2:
                    kpi_card("MOYENNE BONUS", f"{avg_bonus:.1f}", "POINTS DOUBLÉS", "#FFF")
                with k3:
                    kpi_card("MEILLEUR BONUS", int(best_bonus), "RECORD", C_GOLD)
                ...
                st.dataframe(
                    df_bonus[['Player', 'Pick', 'Score', 'Gain']].sort_values('Pick', ascending=False),
                    hide_index=True,
                    use_container_width=True,
                    column_config={
            ...
            # V12.0: discipline NoPick sur 15 jours
            nopicks_15 = df_15[df_15['Score'] == 0].groupby('Player').size().sort_values(ascending=False)
            ...
            def render_row(title, sub, hot, cold, metric, unit=""):
                ...
                with c1:
                    ...
                    if hot.empty:
                        h += "<div style='color:#666'>Aucune donnée</div>"
                    else:
            ...
            render_row(
                "DYNAMIQUE DE FORME (15J VS SAISON)",
            ...
            render_row(
                ...
                delta_rank.sort_values(ascending=False),
                delta_rank.sort_values(),
                "diff",
                "places"
            )

            # --- V12.0: DISCIPLINE (NO PICK) ---
            render_row(
                "DISCIPLINE (NO PICK / 0)",
                "Nombre de picks à 0 point sur les 15 derniers jours.",
                nopicks_15,
                nopicks_15.sort_values(),
                "int",
                "NoPick"
            )
            ...
            # V12.0: IRON MAN (plus grand nombre de matchs joués)
            iron = full_stats.sort_values('Games', ascending=False).iloc[0]

            def hof_card(title, icon, color, p_name, val, unit, desc):
                return f"""
                <div class="glass-card" style="position:relative; overflow:hidden">
                    <div style="position:absolute; right:-10px; top:-10px; font-size:5rem; opacity:0.05; pointer-events:none">
                        {icon}
                    </div>
            ...
            with c1:
                st.markdown(
                    hof_card("THE GOAT", "🏆", C_GOLD, sniper['Player'], f"{sniper['Moyenne']:.1f}", "PTS MOY", "Meilleure moyenne générale de la saison"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("REAL MVP", "💎", C_PURE, pure_avg['Player'], f"{pure_avg['Moyenne_Raw']:.1f}", "PTS MOY (BRUT)", "Meilleure moyenne sans compter les bonus"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("THE SNIPER", "🎯", C_PURPLE, sniper_bp['Player'], int(sniper_bp['BP_Count']), "BEST PICKS", "Plus grand nombre de 'Best Pick' TTFL trouvés"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("ALPHA DOG", "🐺", C_ALPHA, alpha_dog['Player'], int(alpha_dog['Alpha_Count']), "TOPS TEAM", "Plus grand nombre de fois meilleur scoreur de l'équipe"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("HUMAN TORCH", "🔥", "#FF5252", torche['Player'], f"{torche['Last15']:.1f}", "PTS / 15J", "Meilleure moyenne sur les 15 derniers jours"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("RISING STAR", "🚀", C_GREEN, progression['Player'], f"+{progression['ProgressionPct']:.1f}%", "PROGRESSION", "Plus forte hausse (Moy. 15j vs Saison)"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("THE CEILING", "🏔️", "#A78BFA", peak['Player'], int(peak['Best']), "PTS MAX", "Record de points sur un seul match (Bonus inclus)"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("PURE SCORER", "🏀", "#F472B6", pure_peak['Player'], int(pure_peak['Best_Raw']), "PTS MAX (BRUT)", "Plus gros score réalisé sans bonus"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("THE ALCHEMIST", "⚗️", C_BONUS, alchemist['Player'], int(alchemist['Bonus_Gained']), "PTS BONUS", "Plus grand nombre de points gagnés grâce aux bonus"),
                    unsafe_allow_html=True
                )

            with c2:
                st.markdown(
                    hof_card("BAD BUSINESS", "💸", "#666", bad_business['Player'], int(bad_business['Bonus_Gained']), "PTS BONUS", "Le moins de points gagnés grâce aux bonus"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("ZEN MASTER", "🧘", "#EAB308", zen_master['Player'], f"{int(zen_master['ReliabilityPct'])}%", "FIABILITÉ", "Plus haut % de matchs sans carotte"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("HEAVY HITTER", "🥊", "#64B5F6", heavy['Player'], int(heavy['Count40']), "PICKS >40", "Nombre de scores supérieurs à 40 points"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("NUCLEAR", "☢️", "#EF4444", nuke['Player'], int(nuke['Nukes']), "BOMBS", "Nombre de scores supérieurs à 50 points"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("THE ROCK", "🛡️", C_GREEN, rock['Player'], int(rock['Count30']), "MATCHS", "Nombre de scores > 30 pts (Assurance tous risques)"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("UNSTOPPABLE", "⚡", "#FBBF24", intouch['Player'], int(intouch['Streak30']), "SERIE", "Plus longue série de matchs consécutifs > 30 pts"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("THE BRICK", "🏗️", "#71717A", brick_layer['Player'], int(brick_layer['Worst_Raw']), "PTS MIN (BRUT)", "Score le plus bas enregistré sans bonus"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("CRASH TEST", "💥", C_ORANGE, crash_test['Player'], int(crash_test['Worst_Bonus']), "PTS MIN (X2)", "Le pire score réalisé avec un bonus"),
                    unsafe_allow_html=True
                )
                st.markdown(
                    hof_card("THE FARMER", "🥕", "#F97316", lapin['Player'], int(lapin['Carottes']), "CAROTTES", "Nombre de scores inférieurs à 20 points"),
                    unsafe_allow_html=True
                )
                # V12.0: IRON MAN
                st.markdown(
                    hof_card("IRON MAN", "🦾", C_IRON, iron['Player'], int(iron['Games']), "MATCHS", "Plus grand nombre de picks joués cette saison"),
                    unsafe_allow_html=True
                )
            ...
            if not st.session_state["admin_access"]:
                ...
                with c2:
                    ...
                    if st.button("🚀 ENVOYER RAPPORT", type="primary"):
                        res = send_discord_webhook(
                            day_df,
                            latest_pick,
                            "https://dino-fant-tvewyye4t3dmqfeuvqsvmg.streamlit.app/"
                        )
                        if res == "success":
                            st.success("✅ Envoyé !")
                        else:
